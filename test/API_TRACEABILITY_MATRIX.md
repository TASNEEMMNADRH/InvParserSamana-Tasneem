  ...
  |--------------|---------------------|----------|-----------|
  | **POST /extract** | Successful extraction from a valid PDF and JSON response returned | `test/test_extract_new.py` | `TestNewExtract.test_endpoint_success_case` |
  | **POST /extract** | Validation failure when document confidence is below threshold | `test/test_extract_new.py` | `TestNewExtract.test_extract_failure_low_confidence` |
  | **POST /extract** | OCI Document AI service unavailable → returns 503 | `test/test_extract_new.py` | `TestNewExtract.test_extract_failure_oci_down` |
  | **POST /extract** | Database save failure handled by API → returns 503 | `test/test_extract_new.py` | `TestNewExtract.test_extract_failure_db_save` |
  | **POST /extract** | Edge case: extraction returns empty fields → 200 with empty payload | `test/test_extract_new.py` | `TestNewExtract.test_extract_empty_document_fields_returns_empty_data` |
